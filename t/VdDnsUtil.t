...
# ========================================
# VdDnsUtil::delete_records()のテスト
# ========================================
{
    my $records = "a www 192.168.1.1\na mail 192.168.1.2";
    is(
        VdDnsUtil::delete_records($records, 'a mail 192.168.1.2'),
        'a www 192.168.1.1',
        'delete_records: 完全一致する行を削除できる',
    );
}
{
    my $records = "a www 192.168.1.1\na mail 192.168.1.2";
    is(
        VdDnsUtil::delete_records($records, 'a mail'),
        "a www 192.168.1.1\na mail 192.168.1.2",
        'delete_records: 部分一致では削除されない',
    );
}
{
    my $records = "a www 192.168.1.1\ntxt _acme-challenge \"abc\"\na mail 10.0.0.1";
    is(
        VdDnsUtil::delete_records($records, 'txt _acme-challenge "abc"'),
        "a www 192.168.1.1\na mail 10.0.0.1",
        'delete_records: 中間行を削除できる',
    );
}
{
    my $records = "txt _acme-challenge \"abc\"\ntxt _acme-challenge \"abc\"\na www 192.168.1.1";
    is(
        VdDnsUtil::delete_records($records, 'txt _acme-challenge "abc"'),
        'a www 192.168.1.1',
        'delete_records: 完全一致する行が複数あれば全て削除する',
    );
}
{
    my $records = "a www 192.168.1.1\na mail 192.168.1.2";
    is(
        VdDnsUtil::delete_records($records, 'a none 0.0.0.0'),
        "a www 192.168.1.1\na mail 192.168.1.2",
        'delete_records: 一致する行がなければ元のまま',
    );
}
