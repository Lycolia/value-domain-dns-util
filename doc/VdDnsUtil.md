# [`VdDnsUtil.pm`] ValueDomain DNS APIのツールキット本体
...
```perl
my $exists = VdDnsUtil::find_first_record($records, "txt $certbot_domain");
...
# $record はDNSレコード一行分
my $new_records = VdDnsUtil::append_record($records, $record);
```
...
my $new_records = VdDnsUtil::replace_record($source_records, "txt $acme_domain", $new_record);
```

### `delete_records($records, $subject)`

Value-DomainのDNSレコードデータ（records）にあるレコードのうち、`$subject`と完全一致する行を全削除する。
...
| ---------- | --------------------------------------- | --------------------------------------------------------------------- |
| `$records` | APIのレスポンスにある`.results.records` |                                                                       |
| `$subject` | 削除対象のレコード行（完全一致）        | `txt _acme-challenge.hoge "abcdef12345"`のように1行まるごとの文字列。 |

**戻り値**

`$records`から`$subject`に完全一致する行を取り除いた文字列。
...
```perl
my $source_ttl   = 60;
my $adjusted_ttl = VdDnsUtil::adjust_ttl($source_ttl);  # => 120

my $source_ttl   = 130;
my $adjusted_ttl = VdDnsUtil::adjust_ttl($source_ttl);  # => 130
```
